...
parser.add_argument('--out_path', type=str, help='output paths')
...
if __name__ == '__main__':
    ...
    if not os.path.exists(os.path.join(opt.out_path, 'A')):
        os.makedirs(os.path.join(opt.out_path, 'A'))
    if not os.path.exists(os.path.join(opt.out_path, 'B')):
        os.makedirs(os.path.join(opt.out_path, 'B'))
    ...
    for i, batch in enumerate(dataloader):
        ...
        # Save image files
        save_image(fake_A, os.path.join(opt.out_path, f'A/{i+1:04d}.png'))
        save_image(fake_B, os.path.join(opt.out_path, f'B/{i+1:04d}.png'))
